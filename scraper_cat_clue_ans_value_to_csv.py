...
def scrape_game(game_id):
    ...
    values = soup.select('.clue_value')  # Clue values
    ...
    for i, clue in enumerate(clues):
        ...
        value = values[i].text if i < len(values) else None
    

        if question_element and answer_element:
            question = question_element.text
            answer = answer_element.text
            qa_pairs.append([category, question, answer, value]) 
...
def write_to_csv(qa_pairs, filename):  
    ...
    with open(filename, 'w', newline='', encoding='utf-8') as file:
        ...
        writer.writerow(["Category", "Clue", "Answer", "Value"]) 
